...
- **影响**: 超长路径（>1000点）可能卡顿
- **修复**: 
  1. 已优化距离计算（问题#16），消除二次遍历
  2. 新增NumPy向量化距离计算函数（10-100倍性能提升）
- **状态**: ✓ 已优化 - 添加了`_calculate_distances_vectorized()`函数
- **特性**: 
  - 自动检测NumPy是否可用
  - 可用时使用向量化计算（快速）
  - 不可用时回退到循环（兼容）
...
### 14. 打卡点序号从1开始（设计问题）✓ 已修复
- **位置**: main.py 行4227
- **问题**: target_sequence从1开始而非0
- **风险**: 与Python惯例不符，容易出现off-by-one错误
- **影响**: 所有使用target_sequence的地方都需要-1
- **修复**: 已改为0-based索引（0表示第一个打卡点）
- **状态**: ✓ 已修复
- **改动**: 
  - 初始化改为0而非1
  - 所有边界检查改为 `0 <= index < len()`
  - 直接使用target_sequence作为数组索引
  - 日志显示时转换为1-based（用户友好）
...
### 17. 代码重复（ApiClient._request方法）✓ 已修复
...
### 代码质量修复（10项）
5. ✓ **问题#7: hashlib重复导入** - 已删除
6. ✓ **问题#8: Semaphore->Lock** - 性能优化
7. ✓ **问题#9: 距离计算** - Haversine公式，精度提升
8. ✓ **问题#10: 经度系数** - Haversine自动处理
9. ✓ **问题#11: 嵌套循环** - NumPy向量化优化
10. ✓ **问题#12: 速度随机** - 移动平均平滑
11. ✓ **问题#13: 浮点数比较** - epsilon容差比较
12. ✓ **问题#14: target_sequence** - 改为0-based索引
13. ✓ **问题#16: 距离重复计算** - 生成时同步计算
14. ✓ **问题#17: 代码重复** - 已删除重复代码
...
9. ✓ 问题#9: 距离计算精度（已完成）
10. ✓ 问题#10: 经度系数（已完成）
11. ✓ 问题#11: 嵌套循环（已完成 - NumPy向量化）
12. ✓ 问题#12: 速度随机（已完成 - 移动平均平滑）
13. ✓ 问题#13: 浮点数比较（已完成）
14. ✓ 问题#14: 序号从1开始（已完成 - 改为0-based）
15. 📝 问题#15: 打卡点距离（需验证）
16. ✓ 问题#16: 距离重复计算（已完成）
17. ✓ 问题#17: 删除重复代码（已完成）
18. 🟡 问题#18: 注释代码（待处理）
