...
def _buffer_log(level, message): #暂存日志到缓冲区，同时打印到控制台。 level: 'INFO', 'WARNING', 'ERROR' 等
    print(message) # 保证控制台依然能看到
    _log_buffer.append((level, message)) # 存入缓冲区

def _try_import_builtin(module_name, display_name=None, use_print=False): #尝试导入内置模块，失败时记录但不立即退出。
...
def import_standard_libraries(): #逐步导入所有必需的标准库。 收集所有导入失败的模块，最后统一报告，便于用户一次性解决所有问题。
    logging.info("=" * 80)
...
def import_core_third_party(): #导入 'check_and_import_dependencies' 未涵盖的核心第三方库。 收集所有导入失败的模块，最后统一报告。
    logging.info("=" * 80)
...
def check_and_import_dependencies(): #检查并导入所有主要的第三方库 (Flask, requests, playwright 等)。 收集所有导入失败的模块，最后统一报告并提供安装命令。
    logging.info("=" * 80)
...
def initialize_global_variables(): #初始化所有全局变量和应用状态。 这应该在所有库都导入成功之后运行。
...
class NoColorFileFormatter(logging.Formatter):
    ...
    def format(self, record): #格式化日志记录并移除颜色代码。  处理流程： 1. 调用父类的format方法，生成可能包含ANSI颜色代码的日志消息 2. 使用正则表达式替换所有ANSI转义序列为空字符串 3. 返回清理后的纯文本消息  参数: record: LogRecord对象，包含日志信息（级别、消息、时间等）  返回: str: 不含ANSI颜色代码的格式化日志消息
...
class CustomLogHandler(logging.FileHandler):
    ...
    def emit(self, record): #写入日志记录前检查文件大小。
        try:
    ...
    def do_rollover(self): #执行标准的日志轮转。 实现逻辑： 1. 检查 zx-slm-tool-09.log 是否存在，如果存在则删除 2. 将 zx-slm-tool-08.log 重命名为 zx-slm-tool-09.log 3. 将 zx-slm-tool-07.log 重命名为 zx-slm-tool-08.log 4. ... 5. 将 zx-slm-tool-01.log 重命名为 zx-slm-tool-02.log 6. 最后将 zx-slm-tool.log 重命名为 zx-slm-tool-01.log 7. 创建一个新的 zx-slm-tool.log 文件用于后续写入
        if self.stream:
            self.stream.close()
            self.stream = None
...
def archive_old_logs(): #归档旧的日志文件。  在程序启动时调用，将上一次运行的日志文件压缩并移动到归档目录。 支持归档 zx-slm-tool.log 和 zx-slm-tool-*.log 格式的文件。
...
def cleanup_archive_directory(archive_dir, max_size_mb): #清理归档目录，确保不超过指定大小。  当归档目录大小超过限制时，删除最早的归档文件。  参数: archive_dir: 归档目录路径 max_size_mb: 最大大小（MB），0表示不限制
    if max_size_mb <= 0:
        return  # 不限制大小
...
def setup_logging(): #配置详细的日志系统（带自定义轮转逻辑）。  功能： 1. 启动时归档旧日志文件到 logs/archive（压缩为 zip） 2. 当 zx-slm-tool.log 大小超过配置限制时自动轮转 3. 清理归档目录，防止占用过多磁盘空间  配置项（config.ini [Logging]）： - log_rotation_size_mb: 单个日志文件最大大小（MB） - archive_max_size_mb: 归档目录最大大小（MB），0表示不限制
...
def auto_init_system(): #自动初始化系统，创建所有必需的文件和目录。
    logging.info("=" * 80)
...
def _create_directories(): #创建程序运行所需的目录结构，目录路径从 config.ini 配置文件读取。  目录说明： - logs: 日志文件存储目录（可在 config.ini [Logging] log_dir 配置） - school_accounts: 学校账号信息存储（可在 config.ini [System] school_accounts_dir 配置） - system_accounts: 系统认证账号存储（可在 config.ini [System] system_accounts_dir 配置） - sessions: 用户会话持久化存储（可在 config.ini [System] sessions_dir 配置） - tokens: API令牌存储目录（可在 config.ini [System] tokens_dir 配置）  注意：此函数会更新全局变量，使其他模块可以使用配置的路径。
...
def _get_default_config(): #获取默认配置项字典。  此函数返回一个ConfigParser对象，包含所有配置节和默认值。 主要用途： 1. 创建新的config.ini文件 2. 补全旧配置文件中缺失的配置项（版本升级场景）  配置结构： - [Admin]: 超级管理员设置 - [Guest]: 游客访问控制 - [System]: 系统路径和文件配置 - [Security]: 安全策略（密码存储、防暴力破解） - [Map]: 地图API配置（高德地图密钥）  返回: configparser.ConfigParser: 包含默认配置的对象  注意：修改这里的默认值会影响新用户的初始配置
    config = configparser.ConfigParser()
...
def _write_config_with_comments(config_obj, filepath): #将配置写入文件，包含详细的中文注释。  由于ConfigParser不保留注释，这个函数手动写入带注释的配置文件。
...
def _create_config_ini(): #创建或更新config.ini配置文件（兼容旧版本，自动补全缺失参数）
...
def _create_permissions_json(): #创建默认的permissions.json权限配置文件
...
def _create_default_admin(): #创建默认的管理员账号
...
class AuthSystem:
    ...
    def _load_config(self): #加载配置文件（增强：处理重复项错误并提示用户修复）
    ...
    def find_user_by_phone(self, phone): #遍历所有系统账号，查找绑定了特定手机号的用户。  参数: phone (str): 要查找的手机号。  返回: str: 绑定的用户名，如果未找到则返回 None。
    ...
    def unbind_phone_from_user(self, phone, except_username=None): #查找绑定了指定手机号的用户，并将其手机号字段清空。  参数: phone (str): 要解绑的手机号。 except_username (str, 可选): 如果找到的用户是此用户，则不解绑。
        if not phone:
            return
    ...
    def _load_permissions(self): #加载权限配置
    ...
    def _save_permissions(self): #保存权限配置
    ...
    def get_user_file_path(self, auth_username): #获取用户文件路径
    ...
    def _update_user_file_group(self, username, new_group): #(辅助函数) 更新 system_accounts 中用户JSON文件内的 group 字段。 这确保了 permissions.json 和用户文件之间的数据一致性。
    ...
    def _get_password_storage_method(self): #获取密码存储方式配置。  从config.ini的[Security]节读取password_storage配置项： - 'plaintext': 明文存储（默认，不安全） - 'encrypted': SHA256哈希存储  ⚠️ 安全警告： - 默认为plaintext，密码完全可见 - encrypted模式使用SHA256但不加盐，仍不够安全  💡 建议改进： - 添加bcrypt或argon2选项 - 强制encrypted模式，禁止plaintext - 密码哈希应使用慢速算法+随机盐  返回: str: 'plaintext' 或 'encrypted'
    ...
    def _encrypt_password(self, password): #加密密码（根据配置决定是否加密）。  加密方式： - plaintext: 不加密，直接返回原密码（已废弃，不推荐） - encrypted: 使用SHA256哈希（已废弃，仅用于向后兼容） - bcrypt: 使用bcrypt算法（推荐，自动加盐，抗暴力破解）  ⚠️ 安全说明： 1. bcrypt自动生成随机盐并嵌入到哈希中 2. bcrypt计算慢，抗GPU/ASIC暴力破解 3. 每次加密相同密码产生不同哈希（因为盐不同） 4. SHA256模式仅用于向后兼容旧密码  参数: password (str): 明文密码  返回: str: 加密后的密码（或明文）  示例: >>> _encrypt_password("admin") # bcrypt模式: "$2b$12$abcd..."（60字符） # SHA256模式（旧）: "8c6976e5b5..."（64字符） # plaintext模式（极不安全）: "admin"
    ...
    def _verify_password(self, input_password, stored_password): #验证用户输入的密码是否正确。  验证流程： 1. 自动检测存储密码的格式（bcrypt/SHA256/plaintext） 2. bcrypt模式：使用bcrypt.checkpw()验证（推荐） 3. encrypted模式：使用secrets.compare_digest()常量时间比较 4. plaintext模式：使用secrets.compare_digest()比较（不推荐）  ✓ 安全改进： 1. 使用secrets.compare_digest()防止时序攻击 2. 支持bcrypt自动验证（包含盐值检查） 3. 常量时间比较，响应时间不泄露密码信息 4. 自动检测密码格式，兼容旧密码  参数: input_password (str): 用户输入的密码（明文） stored_password (str): 存储的密码（明文/SHA256哈希/bcrypt哈希）  返回: bool: True表示密码正确，False表示密码错误  安全特性： - 无论密码对错，响应时间基本相同（常量时间） - 防止通过响应时间分析推测密码
        logging.debug(
    ...
    def get_login_history(self, username=None, limit=100): #获取登录历史
    ...
    def check_brute_force(self, auth_username, ip_address): #检查暴力破解（5分钟内最多5次失败）
        recent_attempts = []
    ...
    def generate_2fa_secret(self, auth_username): #生成2FA密钥
        try:
    ...
    def enable_2fa(self, auth_username, verification_code): #启用2FA（需要验证一次）
        try:
    ...
    def verify_2fa(self, auth_username, verification_code): #验证2FA代码
        try:
    ...
    def check_permission(self, auth_username, permission): #检查用户是否有特定权限（支持差分化权限）  权限计算顺序： 1. 获取用户所属权限组的基础权限 2. 应用用户的自定义权限（added/removed）  重要说明： 此方法已完全支持 user_custom_permissions 差分授权！ - 即使用户所在组没有某权限，也可以通过 user_custom_permissions.added 单独授予 - 即使用户所在组有某权限，也可以通过 user_custom_permissions.removed 单独撤销  示例：用户 zelly 在 user 组（默认无 use_multi_account_button 权限） 通过在 permissions.json 中添加： { "user_custom_permissions": { "zelly": { "added": ["use_multi_account_button"], "removed": [] } } } 该用户即可获得该权限，check_permission 会正确返回 True。
    ...
    def get_user_permissions(self, auth_username): #获取用户的完整权限列表（包含差分权限）
        group = self.get_user_group(auth_username)
    ...
    def set_user_custom_permission(self, auth_username, permission, grant): #为用户设置自定义权限（差分化存储）  Args: auth_username: 用户名 permission: 权限名 grant: True=授予权限, False=移除权限
        with self.lock:
    ...
    def get_user_group(self, auth_username): #获取用户所属组
        super_admin = self.config.get("Admin", "super_admin", fallback="admin")
        if auth_username == super_admin:
            return "super_admin"
        return self.permissions["user_groups"].get(auth_username, "guest")

    def update_user_group(self, auth_username, new_group): #更新用户组（需要管理员权限）
    ...
    def create_permission_group(self, group_name, permissions, display_name): #创建新的权限组（需要超级管理员权限）
        with self.lock:
    ...
    def update_permission_group(self, group_name, permissions): #更新权限组（需要超级管理员权限）  注意：系统预设权限组可以修改，但不能删除
        with self.lock:
    ...
    def delete_permission_group(self, group_name): #删除权限组（需要超级管理员权限）  注意：不允许删除系统预设权限组（guest、user、admin、super_admin）
        with self.lock:
            if group_name not in self.permissions["permission_groups"]:
                return {"success": False, "message": "权限组不存在"}
    ...
    def get_all_groups(self): #获取所有权限组
        return self.permissions["permission_groups"]

    def link_session_to_user(self, auth_username, session_id): #关联会话ID到用户账号（用于状态恢复）
    ...
    def get_user_sessions(self, auth_username): #获取用户关联的会话ID列表
        if auth_username == "guest":
            return []
    ...
    def unlink_session_from_user(self, auth_username, session_id): #取消会话与用户的关联
        if auth_username == "guest":
            return
    ...
    def reset_user_password(self, auth_username, new_password): #重置用户密码（管理员功能）
        with self.lock:
            user_file = self.get_user_file_path(auth_username)
            if not os.path.exists(user_file):
                return {"success": False, "message": "用户不存在"}
    ...
    def update_user_avatar(self, auth_username, avatar_url): #更新用户头像
        with self.lock:
            user_file = self.get_user_file_path(auth_username)
            if not os.path.exists(user_file):
                return {"success": False, "message": "用户不存在"}
    ...
    def update_user_theme(self, auth_username, theme): #更新用户主题偏好
        with self.lock:
            user_file = self.get_user_file_path(auth_username)
            if not os.path.exists(user_file):
                return {"success": False, "message": "用户不存在"}
    ...
    def update_max_sessions(self, auth_username, max_sessions): #更新用户最大会话数量  Args: auth_username: 用户名 max_sessions: 最大会话数 (1=单会话, >1=指定数量, -1=无限制)
        with self.lock:
            user_file = self.get_user_file_path(auth_username)
            if not os.path.exists(user_file):
                return {"success": False, "message": "用户不存在"}
    ...
    def ban_user(self, auth_username): #封禁用户
        with self.lock:
            user_file = self.get_user_file_path(auth_username)
            if not os.path.exists(user_file):
                return {"success": False, "message": "用户不存在"}
    ...
    def unban_user(self, auth_username): #解封用户
        with self.lock:
            user_file = self.get_user_file_path(auth_username)
            if not os.path.exists(user_file):
                return {"success": False, "message": "用户不存在"}
    ...
    def delete_user(self, auth_username): #删除用户（需要管理员权限）
        with self.lock:
            super_admin = self.config.get("Admin", "super_admin", fallback="admin")
            if auth_username == super_admin:
    ...
    def get_user_details(self, auth_username): #获取用户详细信息
        user_file = self.get_user_file_path(auth_username)
        if not os.path.exists(user_file):
            return None
    ...
    def check_single_session_enforcement(self, auth_username, new_session_id): #检查并强制执行会话数量限制  根据用户的max_sessions设置： - max_sessions = 1: 单会话模式，移除所有旧会话 - max_sessions > 1: 多会话模式，超出时移除最旧的会话 - max_sessions = -1: 无限制模式，不移除任何会话  Returns: tuple: (需要清理的旧会话列表, 提示信息)
        if auth_username == "guest":
            return [], ""  # 游客不受限制
    ...
    def get_audit_logs(self, username=None, action=None, limit=100): #获取审计日志
    ...
    def _synchronize_super_admin_permissions(self): #[启动检查] 同步 config.ini 中的 super_admin 与 permissions.json 中的权限组。  逻辑: 1. (降级) 如果用户在 permissions.json 中是 super_admin，但与 config.ini 不符，则降级为 admin。 2. (升级) 如果 config.ini 中指定的用户在 permissions.json 中不是 super_admin，则升级为 super_admin。 3. (添加) 如果 config.ini 中指定的用户不在 permissions.json 中，则添加。
        logging.info(
...
class TokenManager:
    ...
    def _get_token_file_path(self, username): #获取用户的token文件路径
        file_path = os.path.join(self.tokens_dir, f"{username}.json")
        logging.debug(f"_get_token_file_path: 用户 {username} 的令牌文件: {file_path}")
        return file_path

    def generate_token(self): #生成2048位(256字节)的安全令牌
        token = secrets.token_hex(256)
        logging.debug(f"generate_token: 生成新令牌，长度: {len(token)} 字符")
        return token

    def create_token(self, username, session_id=None): #为用户创建新令牌并存储  新的简化设计：一个用户只有一个token，不再关联session_id session信息由./sessions目录管理，token只负责认证  Args: username: 用户名 session_id: 会话UUID（保留参数以兼容现有调用，但不再存储）  Returns: token: 生成的令牌字符串
    ...
    def verify_token(self, username, session_id, token): #验证令牌是否有效  新的简化设计：只验证用户名和token，不再关联session_id session_id参数保留以兼容现有调用  Args: username: 用户名 session_id: 会话UUID（保留参数以兼容，不再使用） token: 要验证的令牌  Returns: tuple: (is_valid, reason)
        token_file = self._get_token_file_path(username)
    ...
    def get_valid_token_for_session(self, username, session_id=None): #获取指定用户的有效Token（如果存在且未过期）。 如果找到有效Token，会刷新其活动时间。  新的简化设计：不再关联session_id，每个用户只有一个token  Args: username: 用户名 session_id: 会话UUID（保留参数以兼容，不再使用）  Returns: str | None: 如果找到有效Token则返回Token字符串，否则返回None。
        token_file = self._get_token_file_path(username)
    ...
    def refresh_token(self, username, session_id=None): #刷新令牌的过期时间和最后活动时间  新的简化设计：不再关联session_id  Args: username: 用户名 session_id: 会话UUID（保留参数以兼容，不再使用）  Returns: bool: 是否刷新成功
        with self.lock:
            token_file = self._get_token_file_path(username)
    ...
    def invalidate_token(self, username, session_id=None): #使令牌失效（用于登出）  新的简化设计：直接删除用户的token文件  Args: username: 用户名 session_id: 会话UUID（保留参数以兼容，不再使用）
        with self.lock:
            token_file = self._get_token_file_path(username)
    ...
    def get_active_sessions(self, username): #获取用户所有有效的会话  新的简化设计：不再由token文件管理session列表 session信息应从./sessions目录获取  Args: username: 用户名  Returns: list: 空列表（session管理已移至./sessions目录）
        logging.debug(
            f"get_active_sessions: token文件不再管理session列表，请使用session管理功能"
        )
        return []

    def cleanup_expired_tokens(self, username): #清理过期的令牌  新的简化设计：如果token过期，直接删除整个token文件  Args: username: 用户名
        with self.lock:
            token_file = self._get_token_file_path(username)
    ...
    def detect_multi_device_login(self, username, new_session_id): #检测多设备登录  Args: username: 用户名 new_session_id: 新的会话ID  Returns: list: 需要踢出的旧会话ID列表
...
class AccountSession:
    ...
    def log(self, message: str): #为日志自动添加账号前缀
...
class ApiClient:
    ...
    def get_unread_notice_count(self): #获取未读通知数量 (POST, 空body, application/json)
        return self._json(
            self._request(
                "POST",
    ...
    def get_notice_list(self, offset=0, limit=10, type_id=0): #获取通知列表 (POST, 空body, 带URL参数)
    ...
    def mark_notice_as_read(self, notice_id): #将单个通知设为已读 (POST, 空body, 带URL参数)
    ...
    @staticmethod
    def generate_random_ua(): #生成一个随机的、模拟安卓设备的User-Agent字符串
        build_texts = [
            "TD1A.221105.001.A1",
            "TP1A.221005.003",
            "SQ3A.220705.004",
    ...
    def get_roll_call_info(self, roll_call_id, user_id): #获取指定签到活动的信息
    ...
    def submit_attendance(self, payload: dict): #提交签到记录
    ...
    def get_attendance_radius(self): #获取服务器设定的签到半径
...
class Api:
    ...
    def _init_state_variables(self): #初始化或重置应用的所有状态变量
        self.device_ua = ""
    ...
    def set_multi_run_only_incomplete(self, flag: bool): #设置多账号模式下“仅执行未完成”的全局开关，并立即刷新全局按钮。
        try:
    ...
    def log(self, message): #将日志消息通过 WebSocket 发送到前端界面显示
    ...
    def js_log(self, level, message): #接收并记录来自JavaScript的日志
        level = level.upper()
        if level == "INFO":
    ...
    def _get_user_accounts_file(self, auth_username): #获取指定认证用户的 school_accounts 存储文件路径。  参数: auth_username: 认证用户名（system_accounts中的用户名）  返回: 文件路径字符串
        user_accounts_dir = os.path.join(SCHOOL_ACCOUNTS_DIR, "user_accounts")
        return os.path.join(user_accounts_dir, f"{auth_username}.json")

    def _load_user_school_accounts(self, auth_username): #加载指定认证用户的所有 school_account 账户密码和UA。  参数: auth_username: 认证用户名  返回: 字典，格式为 {school_username: {"password": "xxx", "ua": "xxx"}, ...} 或旧格式 {school_username: password, ...}（向后兼容）
    ...
    def _save_user_school_accounts(self, auth_username, accounts_dict): #保存指定认证用户的所有 school_account 账户密码和UA。  参数: auth_username: 认证用户名 accounts_dict: 字典，格式为 {school_username: {"password": "xxx", "ua": "xxx"}, ...}
        if not auth_username or auth_username == "guest":
    ...
    def _get_school_account_password(self, auth_username, school_username): #获取指定认证用户的某个 school_account 的密码。  参数: auth_username: 认证用户名 school_username: 学校账户用户名  返回: 密码字符串，如果不存在则返回 None
        if not auth_username or auth_username == "guest":
            return None
    ...
    def _get_school_account_ua(self, auth_username, school_username): #获取指定认证用户的某个 school_account 的UA。  参数: auth_username: 认证用户名 school_username: 学校账户用户名  返回: UA字符串，如果不存在则返回 None
        if not auth_username or auth_username == "guest":
            return None
    ...
    def _save_config(self, username, password=None, ua=None): #保存指定用户的配置到 user/<username>.ini；当 password 为 None 时保留现有密码；当 ua 为 None 时保留现有 UA。同时更新主 config.ini 的 LastUser 和 amap_js_key。
        logging.debug(
    ...
    def _load_global_config(self): #从主 config.ini 加载全局配置（优先读取新版 Map.amap_js_key，兼容旧版 System.AmapJsKey）
    ...
    def _load_config(self, username): #从.ini文件加载指定用户的配置
    ...
    def _save_account_tag(self, username, tag): #保存账号标记到用户配置文件
        user_ini_path = os.path.join(self.user_dir, f"{username}.ini")
    ...
    def _load_account_tag(self, username): #从用户配置文件加载账号标记
        user_ini_path = os.path.join(self.user_dir, f"{username}.ini")
    ...
    def _get_full_user_info_dict(self): #获取当前用户所有信息的字典
        return {
            k: v for k, v in self.user_data.__dict__.items() if not k.startswith("_")
        }

    def get_initial_data(self): #应用启动时由前端调用，获取初始用户列表和最后登录用户
    ...
    def get_user_sessions(self): #获取当前认证用户的会话列表（供前端调用）
    ...
    def save_amap_key(self, api_key: str): #由JS调用，保存高德地图API Key到主配置文件
        try:
    ...
    def on_user_selected(self, username): #return
        """
        当用户在登录界面选择一个已有用户时调用。
    ...
    def generate_new_ua(self): #生成一个新的UA并保存
    ...
    def load_tasks(self): #加载任务列表（增强：稳健去重 + 并发保护 + 离线模式支持）
    ...
    def _try_parse_dt(self, s): #(已重构为类方法) 尝试将不同格式的时间字符串解析为 datetime，支持多种常见格式和值类型。失败返回 None。
        if not s:
            return None
        try:
    ...
    def _get_task_info_text(self, run: RunData) -> str:
        ...
        def try_parse_dt(s): #尝试将不同格式的时间字符串解析为 datetime，支持多种常见格式和值类型。失败返回 None。
            if not s:
                return None
            try:
                if isinstance(s, (int, float)):
    ...
    def get_task_details(self, index): #获取指定任务的详细信息
    ...
    def set_draft_path(self, coords): #接收前端手动绘制的草稿路径
    ...
    def _calculate_distance_m(self, lon1, lat1, lon2, lat2): #使用Haversine公式精确计算两个GPS坐标点之间的距离（米）。  算法说明： - 使用Haversine公式考虑地球曲率 - 适用于任意距离和任意纬度 - 精度高，适合GPS应用  Haversine公式： a = sin²(Δlat/2) + cos(lat1) * cos(lat2) * sin²(Δlon/2) c = 2 * atan2(√a, √(1-a)) distance = R * c  其中 R = 6371000 米（地球平均半径）  ✓ 改进： 1. 考虑地球曲率，精度更高 2. 适用于任意纬度（赤道、极地均可） 3. 适用于任意距离（短距离和长距离） 4. 标准的GPS距离计算方法  参数: lon1 (float): 起点经度（度） lat1 (float): 起点纬度（度） lon2 (float): 终点经度（度） lat2 (float): 终点纬度（度）  返回: float: 距离（米）  示例: >>> _calculate_distance_m(120.0, 30.0, 120.01, 30.01) 1536.8  # 约1.5公里（更精确）
        R = 6371000
    ...
    def _calculate_distances_vectorized(self, coords): #✓ 使用NumPy向量化计算多个坐标点之间的距离（性能优化，问题#11）  参数: coords: [(lon, lat, interval_ms), ...] 坐标列表  返回: float: 总距离（米）  说明: 当NumPy可用时，使用向量化操作一次性计算所有相邻点的距离， 比逐对计算快10-100倍（取决于点数）。如果NumPy不可用，回退到循环计算。
        if len(coords) < 2:
            return 0.0
    ...
    def _gps_random_offset(self, lon, lat, params): #对GPS坐标添加随机偏移，模拟真实GPS的漂移误差。  功能说明： 真实GPS定位存在误差（通常5-10米），为了让模拟轨迹更真实， 需要在关键点之间的插值点上添加随机偏移，避免完美的直线。  偏移计算： 1. 从params中读取偏移范围（location_random_m，单位：米） 2. 在[-m, m]范围内随机选择偏移量 3. 将米转换为经纬度偏移： - 经度偏移 = 米 / 102834.74 - 纬度偏移 = 米 / 111712.69  使用场景： - 关键点（打卡点）：不添加偏移，保持精确坐标 - 插值点：添加偏移，模拟GPS漂移  转换系数说明： - 同_calculate_distance_m，基于纬度30度的近似值 - 经度转换随纬度变化，但对小偏移影响不大  ⚠️ 注意： 1. 偏移是独立的，经度和纬度分别随机 2. 实际GPS误差可能有方向性（如受建筑遮挡） 3. 偏移量过大会导致轨迹看起来不自然  参数: lon (float): 原始经度 lat (float): 原始纬度 params (dict): 参数字典，必须包含'location_random_m'键  返回: tuple: (偏移后的经度, 偏移后的纬度)  示例: >>> params = {'location_random_m': 5}  # 5米偏移 >>> _gps_random_offset(120.0, 30.0, params) (120.00003, 30.00004)  # 偏移约5米
    ...
    def process_path(self): #处理草稿路径，生成带有时间戳的模拟运动轨迹。  功能概述： 将用户在地图上绘制的草稿路径（draft_coords）转换为详细的 模拟跑步轨迹（run_coords），包含坐标、时间间隔、速度变化。  算法流程： 1. 读取草稿路径（关键点序列） 2. 计算每个时间间隔的移动距离（速度×时间） 3. 沿着草稿路径插值生成密集的坐标点 4. 为非关键点添加GPS随机偏移，模拟真实GPS漂移 5. 记录每个点的时间戳（累积时间）  数据结构： - draft_coords: [(lon, lat, is_key_point), ...] - is_key_point=1表示打卡点，必须精确经过 - is_key_point=0表示普通路径点，可以添加偏移  - run_coords: [(lon, lat, interval_ms), ...] - interval_ms: 距离上一个点的时间间隔（毫秒） - 第一个点的interval_ms为0  模拟参数（从self.params读取）： - interval_ms: 上报间隔（毫秒），如5000 = 5秒上报一次 - interval_random_ms: 间隔随机变化范围 - speed_mps: 平均速度（米/秒），如3 m/s = 10.8 km/h - speed_random_mps: 速度随机变化范围 - location_random_m: GPS随机偏移范围（米）  算法特点： 1. 真实性：速度、间隔、位置都添加随机性 2. 精确性：保证经过所有打卡点 3. 连续性：轨迹在草稿路径上连续移动 4. 可控性：通过参数控制运动特征  ⚠️ 潜在问题： 1. 循环嵌套较深，大量坐标点时性能较差 2. 浮点数累积误差可能导致总距离略有偏差 3. 速度随机性可能导致局部速度异常（过快或过慢）  💡 优化建议： 1. 对超长路径进行分段处理 2. 缓存距离计算结果 3. 添加速度平滑算法（避免突变）  返回: dict: { "success": True, "run_coords": [(lon, lat, interval_ms), ...], "total_dist": 总距离（米）, "total_time": 总时长（秒） }
    ...
    def start_single_run(self): #开始执行单个任务
    ...
    def stop_run(self): #停止当前所有正在执行的任务
    ...
    def get_run_status(self): #获取当前运行状态（用于前端轮询）
    ...
    def _submission_worker_loop(self): #后台工作线程：从队列取出提交任务并串行执行。
    ...
    def _finalize_run(self, run_data: RunData, task_index: int, client: ApiClient): #在所有数据提交后，查询服务器确认任务是否已标记为完成
        log_func = (
            client.app.log if hasattr(client.app, "log") else client.app.api_bridge.log
        )
        log_func("正在确认任务状态...")
    ...
    def _get_path_for_distance(self, path, cumulative_distances, target_dist): #如果路径总长不足，则通过来回走的方式凑足目标距离
    ...
    def _get_point_at_distance(self, path, cumulative_distances, dist): #在一条路径上，根据距离找到精确的坐标点
    ...
    def auto_generate_path_with_api(self, api_path_coords, min_t_m, max_t_m, min_d_m): #接收由前端JS API规划好的路径点，并生成模拟数据
        logging.info(
    ...
    def start_all_runs(self, ignore_completed, auto_generate): #开始执行所有符合条件的任务
        logging.info(
    ...
    def _run_all_tasks_manager(self, queue: collections.deque, auto_gen_enabled: bool): #管理“执行所有”任务队列的线程函数
        self.log("开始执行所有任务。")
        if self.window:
    ...
    def get_task_history(self, index): #获取任务的历史跑步记录
        logging.info(
    ...
    def get_historical_track(self, trid): #根据轨迹ID获取历史轨迹坐标点
        logging.info(
    ...
    def update_param(self, key, value): #更新并保存单个参数
    ...
    def get_params(self): #获取当前参数配置  功能说明： - 用于前端获取所有参数配置，包括主题颜色、主题风格等 - 在多账号模式下返回全局参数 - 在单账号模式下返回当前账号的参数  返回: dict: 包含所有参数的字典
        try:
            if self.is_multi_account_mode:
    ...
    def export_task_data(self): #导出当前任务数据为JSON文件（Web模式：返回JSON数据让前端下载）
    ...
    def import_task_data(self, json_data=None): #导入JSON任务数据，进入离线调试模式（UA=Null，保留用户信息）
    ...
    def clear_current_task_draft(self): #清除当前任务的草稿路径和已生成路径
        logging.info(
    ...
    def enter_multi_account_mode(self): #切换到多账号模式（增强：先打断单账号运行）
        try:
            if hasattr(self, "stop_run_flag") and isinstance(
                self.stop_run_flag, threading.Event
            ):
    ...
    def exit_multi_account_mode(self): #退出多账号模式，返回单用户登录页（增强：彻底停止所有执行）
        try:
    ...
    def enter_single_account_mode(self): #进入单账号模式（增强：先停止多账号运行）
        try:
            self.multi_stop_all_accounts()
    ...
    def exit_single_account_mode(self): #退出单账号模式  功能说明： - 用于移动端退出单账号模式，返回会话选择界面 - 清理当前会话状态，但保留认证信息 - 停止所有正在运行的任务  返回: dict: {"success": True} 表示退出成功
        try:
    ...
    def get_session_mode_info(self): #获取会话模式信息（单账号/多账号），用于页面刷新时恢复状态
        mode_info = {
            "success": True,
            "is_multi_account_mode": getattr(self, "is_multi_account_mode", False),
    ...
    def multi_get_all_config_users(self): #获取所有存在配置文件的用户列表，用于前端便捷添加  功能增强：根据当前认证用户的权限过滤返回结果 - 如果用户有school_accounts权限，则可以看到该认证用户名下管理的所有学校账号 - 如果没有特定权限，则只返回空列表 - 这确保了用户只能访问自己有权限的配置数据  返回: dict: {"users": [用户名列表]}
        all_users = sorted(
            [
                os.path.splitext(f)[0]
                for f in os.listdir(self.user_dir)
    ...
    def multi_load_accounts_from_config(self): #模式一：从所有.ini配置文件加载账号
    ...
    def multi_add_account(self, username, password, tag=None, params=None): #模式二：手动或选择性添加账号  Args: username: 账号用户名 password: 账号密码 tag: 账号标记（备注/标签），可选 params: 额外的参数配置（可选，防止前端传参报错）
    ...
    def multi_remove_account(self, username): #移除一个账号
        if username in self.accounts:
            if (
    ...
    def multi_refresh_all_statuses(self): #(多线程)刷新所有账号的任务状态和统计信息
    ...
    def multi_refresh_single_status(self, username: str): #刷新指定账号的状态与摘要（单账号）
        if username not in self.accounts:
    ...
    def _multi_refresh_worker(self, acc: AccountSession, preserve_status: bool = False): #用于刷新的单个账号工作线程 - preserve_status=True 时：若账号正在运行，不改写 status_text，只更新 name 与 summary
        try:
    ...
    def multi_remove_selected_accounts(self, usernames: list[str]): #根据用户名列表移除多个账号
        if not usernames:
            return self.multi_get_all_accounts_status()
    ...
    def multi_remove_all_accounts(self): #一键移除所有账号
        if not self.accounts:
            return self.multi_get_all_accounts_status()
    ...
    def multi_get_all_accounts_status(self, addition=None): #获取所有账号的当前状态，用于刷新前端UI
        status_list = []
        for acc in self.accounts.values():
            tasks_simple = []
    ...
    def multi_download_import_template(self): #下载导入模板（账号、密码、标记），支持 Web模式直接返回文件内容
        try:
            headers = ["账号", "密码", "标记"]
    ...
    def multi_import_accounts(self, filename, base64_content): #从文件导入账号密码，支持 .xlsx/.xls/.csv（仅Web模式，从内存流处理）  Args: filename: 文件名（从前端传入） base64_content: Base64编码的文件内容（从前端传入）
        try:
    ...
    def multi_export_accounts_summary(self): #导出多账号汇总，支持 Web模式直接返回文件内容
        try:
            headers = [
                "账号",
                "姓名",
    ...
    def multi_get_account_params(self, username): #获取指定账号的参数配置。  功能说明： 在多账号模式下，获取单个账号的完整参数配置字典。 用于Web界面显示账号配置或进行参数修改前的查询。  参数: username (str): 账号用户名  返回: dict: 包含以下字段： - success (bool): 操作是否成功 - params (dict): 账号参数字典（成功时） - message (str): 错误信息（失败时）  使用示例: result = api.multi_get_account_params("user123") if result["success"]: print(result["params"])
        if username in self.accounts:
            return {"success": True, "params": self.accounts[username].params}
        return {"success": False, "message": "账号不存在"}

    def multi_update_account_param(self, username, key, value): #更新指定账号的单个参数值。  功能说明： 动态修改账号的配置参数，支持类型自动转换和配置持久化。 修改后立即保存到配置文件，确保重启后配置不丢失。  类型转换规则： - bool类型：支持多种表示方式（'true', '1', 't', 'yes'等） - 其他类型：根据原始参数类型自动转换（int, float, str等）  参数: username (str): 账号用户名 key (str): 参数键名（必须是已存在的参数） value: 新的参数值（将自动转换为正确类型）  返回: dict: 包含以下字段： - success (bool): 操作是否成功 - message (str): 错误信息（失败时）  异常处理: - ValueError: 类型转换失败 - TypeError: 类型不兼容  ⚠️ 注意事项： 1. 只能修改已存在的参数，不能添加新参数 2. 类型必须与原始参数类型兼容 3. 修改会立即保存到磁盘  使用示例: # 修改布尔参数 api.multi_update_account_param("user1", "auto_run", "true") # 修改数值参数 api.multi_update_account_param("user1", "speed", 1.5)
        if username not in self.accounts:
            return {"success": False, "message": "账号不存在"}
    ...
    def multi_start_single_account(self, username, run_only_incomplete: bool = True): #启动指定账号的任务执行线程。  功能说明： 在多账号模式下启动单个账号的自动任务执行。 创建独立的工作线程，不阻塞主线程和其他账号。  执行流程： 1. 验证账号存在性 2. 检查是否已在运行（避免重复启动） 3. 清除停止标志，准备运行 4. 创建daemon工作线程 5. 更新UI状态  参数: username (str): 要启动的账号用户名 run_only_incomplete (bool): 是否只执行未完成的任务 - True: 跳过已完成的任务，只执行新任务或失败任务 - False: 执行所有任务，包括已完成的（重新执行）  返回: dict: 包含以下字段： - success (bool): 操作是否成功 - message (str): 错误信息（失败时）  线程安全： - 使用stop_event控制线程生命周期 - daemon线程确保主程序退出时自动清理 - multi_run_stop_flag全局停止控制  ⚠️ 注意事项： 1. 重复启动会被拒绝（返回失败） 2. 线程异常会被_multi_account_worker内部捕获 3. 启动后无法修改run_only_incomplete参数
        if username not in self.accounts:
            return {"success": False, "message": "账号不存在"}
        acc = self.accounts[username]
        if acc.worker_thread and acc.worker_thread.is_alive():
    ...
    def multi_stop_single_account(self, username): #停止指定账号的任务执行线程。  功能说明： 向账号的工作线程发送停止信号，优雅地终止任务执行。 使用异步监视器等待线程完全退出后清理资源。  停止流程： 1. 验证账号存在性 2. 检查线程是否在运行 3. 设置stop_event信号 4. 启动监视线程等待工作线程退出 5. 线程退出后更新UI状态为"待命"  设计特点： - 非阻塞：使用独立监视线程，不阻塞主线程 - 优雅停止：通过Event信号，让线程自己退出 - 资源清理：线程退出后自动清理worker_thread句柄 - 状态同步：实时更新UI状态显示  监视器(_watch_stop)机制： - 每200ms检查一次线程状态 - 线程退出后立即清理和更新UI - daemon线程确保不会阻止程序退出 - 捕获所有异常，避免监视器崩溃  ⚠️ 注意事项： 1. 停止是异步的，可能需要几秒钟完成 2. 如果线程未在运行，返回失败但不报错 3. 强制停止可能导致任务状态不完整
    ...
    def multi_stop_all_accounts(self): #停止所有账号的运行
    ...
    def _update_multi_global_buttons(self): #根据当前多账号状态刷新“全部开始/全部停止/返回登录页”按钮的可用性
        session_id = getattr(self, "_web_session_id", None)
        if not session_id or not socketio:
            logging.debug(
    ...
    def _multi_fetch_and_summarize_tasks(self, acc: AccountSession): #(辅助函数) 为单个账号获取任务列表并计算统计信息（按任务ID去重）
        acc.all_run_data = []
    ...
    def _multi_fetch_attendance_stats(self, acc: AccountSession): #(多账号) 获取单个账号的签到统计
        if not acc.user_data.id:
            return  # 尚未登录
    ...
    def multi_path_generation_callback(self, username: str, success: bool, data: any): #接收来自JS的路径规划结果并唤醒对应线程
    ...
    def _get_device_sign_code(self, username): #生成或获取设备标识码 (signCode)
    ...
    def get_cached_notifications(self): #获取缓存的通知（用于快速显示）
    ...
    def mark_notification_read(self, notice_id): #(单账号) 将指定ID的通知设为已读
        logging.info(
    ...
    def _auto_refresh_worker(self): #(单账号) 后台自动刷新通知和签到的线程 (已修复)
    ...
    def _check_and_trigger_auto_attendance(self, context: "Api | AccountSession"): #(辅助函数) 检查并执行单个上下文(Api或AccountSession)的自动签到。
    ...
    def _multi_auto_attendance_worker(self): #(多账号) 后台自动刷新和签到所有账号的线程
...
def resource_path(relative_path): #获取资源文件的绝对路径，兼容 PyInstaller 打包和开发环境
    if hasattr(sys, "_MEIPASS"):
        return os.path.join(sys._MEIPASS, relative_path)
    return os.path.join(os.path.abspath("."), relative_path)


def check_port_available(host, port): #检查端口是否可用
...
def _load_ip_cache(): #启动时加载IP归属地缓存文件
...
def _save_ip_cache(): #保存IP归属地缓存到文件（线程安全）
    with ip_cache_lock:
        try:
...
def update_session_activity(session_id): #更新会话活动时间
    with session_activity_lock:
        session_activity[session_id] = time.time()


def update_session_activity(session_id): #更新会话活动时间（线程安全）
    if not session_id:
        return
    with session_activity_lock:
        session_activity[session_id] = time.time()
...
def cleanup_session(session_id, reason="manual"): #清理指定会话（支持指定原因）
    if not session_id or session_id == "null" or session_id.strip() == "":
...
def cleanup_inactive_session(session_id): #清理不活跃的会话
    if not session_id or session_id == "null" or session_id.strip() == "":
        logging.debug(f"跳过清理无效会话ID: '{session_id}'")
        return
...
def monitor_session_inactivity(): #监控会话不活跃状态并清理 (已优化)  功能说明： 1. 定期检查所有会话。 2. 如果会话有正在执行的跑步任务或自动签到任务，则自动更新其“最后活跃时间”。 3. 如果会话没有活跃任务，则检查其“最后活跃时间”是否超时。 4. 如果超时，则调用 cleanup_inactive_session 清理该会话。
    check_interval = 60
...
def start_session_monitor(): #启动会话不活跃监控
...
def _load_session_index(): #加载会话索引文件
    try:
...
def _save_session_index(index): #保存会话索引文件
    try:
...
def save_session_state(session_id, api_instance, force_save=False): #将会话状态保存到文件（完整版：保存所有应用状态包括离线任务数据）  Args: session_id: 会话UUID api_instance: Api实例 force_save: 强制保存，即使距离上次保存时间很短
...
def load_session_state(session_id): #从文件加载会话状态
    try:
        index = _load_session_index()
...
def cleanup_expired_sessions(): #清理过期的会话文件（7天未访问）  此函数遍历所有会话文件，删除超过7天未访问的会话。
    try:
...
def restore_session_to_api_instance(api_instance, state): #将保存的会话状态恢复到Api实例  Args: api_instance: Api实例 state: 从文件加载的状态字典
    try:
...
def get_captcha_original_width(html_content): #【移动端验证码缩放】获取验证码的原始宽度  功能说明: - 从HTML中解析验证码结构（行数、列数） - 从CSS中提取单个像素的宽度 - 计算总宽度 = 列数 × 单个像素宽度  参数: html_content (str): 验证码HTML内容  返回: float: 验证码原始宽度（像素），解析失败返回None  示例: 原始HTML: 46列 × 10px/像素 = 460px总宽度 原始HTML: 40列 × 12px/像素 = 480px总宽度
    try:
...
def resize_captcha_html(html_content, target_width): #自动分析并调整像素验证码HTML的大小 (Grid Layout 增强版 - 修正Padding/Border计算)。
    try:
...
def load_all_sessions(args): #启动时加载所有持久化会话
    if not os.path.exists(SESSION_STORAGE_DIR):
        return
...
class BackgroundTaskManager:
    ...
    def load_task_state(self, session_id): #从文件加载任务状态
        task_file = self._get_task_file_path(session_id)
    ...
    def get_task_status(self, session_id): #获取任务状态
        with self.lock:
            if session_id in self.tasks:
    ...
    def stop_task(self, session_id): #停止后台任务
        with self.lock:
            if session_id in self.tasks:
    ...
    def cleanup_old_tasks(self, max_age_hours=24): #清理旧的任务状态文件
        try:
            current_time = time.time()
...
class ChromeBrowserPool:
    ...
    def _initialize_for_thread(self): #在每个线程中首次使用时初始化Playwright和浏览器
    ...
    def get_context(self, session_id): #获取或创建指定会话的浏览器上下文（线程安全）
    ...
    def execute_js(self, session_id, script, *args): #在指定会话的Chrome中执行JavaScript代码（线程安全）
        try:
            self._initialize_for_thread()
    ...
    def close_context(self, session_id): #关闭指定会话的浏览器上下文（线程安全）
    ...
    def cleanup_thread(self): #清理当前线程的所有Playwright资源
...
def _cleanup_playwright(): #在程序退出时清理Playwright资源。  功能说明： - 这是一个注册在atexit的清理函数，确保程序退出时正确关闭所有浏览器实例 - 防止遗留的浏览器进程占用系统资源  清理流程： 1. 检查chrome_pool全局实例是否存在 2. 调用chrome_pool.cleanup()关闭所有浏览器 3. 记录清理结果  异常处理： - 使用try-except捕获清理过程中的所有异常 - exc_info=False避免在退出时打印完整堆栈（可能引起混淆） - 只记录错误，不重新抛出，确保程序正常退出  设计考虑： - 使用global访问chrome_pool而不是参数传递，因为atexit不支持参数 - 判空检查避免未初始化时的错误  使用场景： - 正常退出（Ctrl+C、关闭终端） - 异常退出（未捕获的异常） - 系统信号（SIGTERM等）  注意： - 此函数由atexit.register()自动调用，不需手动调用 - 执行时机在Python解释器关闭前
    global chrome_pool
    if chrome_pool:
...
def start_background_auto_attendance(args): #在服务器启动时扫描所有.ini配置文件，为启用自动签到的账号启动后台工作线程。  功能说明： - 自动发现并加载所有启用了auto_attendance_enabled的账号配置 - 根据账号数量智能选择单账号或多账号模式 - 在后台守护线程中持续运行，无需用户干预  工作模式：  **单账号模式**（只有1个启用自动签到的账号）： - 为该账号创建独立的Api实例 - 启动单独的_auto_refresh_worker线程 - 资源占用更少，适合个人使用  **多账号模式**（2个或更多账号）： - 所有账号共享一个Api实例 - 启动_multi_auto_attendance_worker统一管理 - 更高效的资源利用，适合批量管理  执行流程： 1. 扫描SCHOOL_ACCOUNTS_DIR目录中的所有.ini文件 2. 读取每个文件的配置，检查auto_attendance_enabled参数 3. 收集所有启用自动签到的账号信息 4. 根据账号数量选择模式并启动相应的后台线程  参数说明： - args: 命令行参数对象，包含--no-auto-start等配置  异常处理： - 单个账号加载失败不影响其他账号 - 登录失败会记录错误但不终止服务 - 所有异常都会记录到日志中  全局变量： - _background_service_api: 保持Api实例存活的引用（单账号模式） - chrome_pool: 共享的浏览器池（如果需要）  设计考虑： - 使用daemon线程确保主程序退出时自动停止 - 临时创建Api实例仅用于读取配置，避免资源浪费 - 账号信息缓存在内存中，避免重复读取文件  注意事项： - 此函数应在Flask服务器启动前调用 - 需要确保SCHOOL_ACCOUNTS_DIR目录存在且有读权限 - 账号的.ini文件必须包含密码和auto_attendance_enabled配置
    try:
...
def load_ssl_config(): #从config.ini文件加载SSL配置。  功能说明： 1. 读取配置文件中的[SSL]节 2. 解析SSL相关的配置项（启用状态、证书路径、仅HTTPS模式等） 3. 返回配置字典供后续使用  返回值： dict: 包含SSL配置的字典，键包括： - ssl_enabled (bool): 是否启用SSL - ssl_cert_path (str): 证书文件路径 - ssl_key_path (str): 私钥文件路径 - https_only (bool): 是否仅允许HTTPS访问 如果配置文件不存在或读取失败，返回默认配置（SSL禁用）
    config_file = os.path.join(os.path.dirname(__file__), "config.ini")
...
def save_ssl_config(ssl_config): #将SSL配置保存到config.ini文件。  参数： ssl_config (dict): 包含SSL配置的字典，键应包括： - ssl_enabled (bool): 是否启用SSL - ssl_cert_path (str): 证书文件路径 - ssl_key_path (str): 私钥文件路径 - https_only (bool): 是否仅允许HTTPS访问  返回值： bool: 保存成功返回True，失败返回False
    config_file = os.path.join(os.path.dirname(__file__), "config.ini")
...
def validate_ssl_certificate(cert_path, key_path): #验证SSL证书文件和密钥文件是否有效。  功能说明： 1. 检查证书文件和密钥文件是否存在 2. 检查文件是否可读 3. 验证证书格式和有效期（基础验证） 4. 检查证书和密钥是否匹配  参数： cert_path (str): 证书文件路径（相对或绝对路径） key_path (str): 密钥文件路径（相对或绝对路径）  返回值： tuple: (bool, str, dict) - bool: 验证是否通过 - str: 错误消息（验证通过时为空字符串） - dict: 证书信息字典（验证通过时包含证书详细信息，失败时为空）
...
def get_ssl_certificate_info(cert_path): #获取SSL证书的详细信息（如有效期、颁发者、主题等）。  参数： cert_path (str): 证书文件路径  返回值： dict: 包含证书详细信息的字典，失败时返回空字典
    if not os.path.isabs(cert_path):
        cert_path = os.path.join(os.path.dirname(__file__), cert_path)
...
def start_web_server(args_param): #启动Flask Web服务器主函数，集成SocketIO实时通信和Chrome浏览器自动化。
    ...
    def login_required(f): #一个装饰器，用于验证用户是否已登录。 它检查 X-Session-ID，验证会话，并将用户信息存储在 g 对象中。
    ...
    @app.before_request
    def check_ip_ban_before_request(): #全局IP封禁检查拦截器
    ...
    def auth_register(): #用户注册API端点（已升级支持手机号、昵称、头像）。
        try:
    ...
    def auth_guest_login(): #游客登录API - 无需密码的快速访问入口。
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_logout(): #用户登出API - 安全退出系统并清理所有会话数据。  功能说明： - 终止用户的当前会话 - 使token失效（防止重放攻击） - 清除浏览器cookie - 释放服务器端资源  请求格式： - 方法：POST - 路径：/auth/logout - 请求头： * X-Session-ID: 要登出的会话标识符（必需） - 请求体：无需  响应格式： { "success": true, "message": "登出成功" }  响应头： - Set-Cookie: auth_token=; Max-Age=0（清除cookie）  处理流程： 1. 从请求头获取session_id 2. 验证session_id存在性 3. 获取用户信息（用户名、是否游客） 4. 如果是注册用户，使其token失效： - 调用token_manager.invalidate_token() - 记录登出事件到日志 5. 清理服务器端会话： - 从web_sessions移除会话 - 关闭Api实例资源 - 删除会话持久化文件 6. 清除客户端cookie（auth_token） 7. 返回成功响应  安全特性： - Token失效机制：防止token被重复使用 - Cookie清除：防止浏览器自动重新登录 - 资源释放：避免会话泄漏 - 审计日志：记录登出时间和原因  游客处理： - 游客用户没有token，只清理会话即可 - 不记录游客的登出事件  错误情况： - 缺少session_id：返回400错误 - 会话不存在：仍然返回成功（幂等性）  注意： - 此接口幂等性强，重复调用不会出错 - 登出后前端应导航到登录页 - 多标签页的其他标签会话也会失效 - 调用cleanup_session时原因设为"user_logout"  使用场景： - 用户主动点击"退出"按钮 - 会话超时后的强制登出 - 安全策略要求的定期重新认证
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_check_permission(): #权限检查API - 验证用户是否拥有特定权限。  功能说明： - 基于RBAC（基于角色的访问控制）模型 - 检查用户所在组是否有特定权限 - 用于前端UI控制（显示/隐藏功能按钮） - 用于API调用前的权限验证  请求格式： - 方法：POST - 路径：/auth/check_permission - 请求头： * X-Session-ID: 会话标识符（必需） - 请求体： { "permission": "权限名称" }  响应格式： { "success": true/false, "has_permission": true/false }  处理流程： 1. 从请求头获取session_id 2. 从请求体获取permission名称 3. 验证session_id存在且有效 4. 从web_sessions获取Api实例 5. 验证用户已认证（has auth_username） 6. 调用auth_system.check_permission()检查权限 7. 返回检查结果  权限系统： - 权限定义在auth.ini的[Permissions]段 - 格式：permission_name = group1,group2,group3 - 例如：admin_panel = admin - 支持多组共享同一权限  差分授权支持（user_custom_permissions）： - 此API完全支持差分授权功能 - 通过 auth_system.check_permission() 调用，该方法会自动： 1. 检查用户所在组的基础权限 2. 应用 user_custom_permissions 中的 added 权限（授予） 3. 应用 user_custom_permissions 中的 removed 权限（撤销） - 示例：用户在 'user' 组，但通过 user_custom_permissions.added 获得了特定权限 { "user_custom_permissions": { "zelly": { "added": ["use_multi_account_button"], "removed": [] } } } 该用户调用 /auth/check_permission 检查 use_multi_account_button 时会返回 true  常见权限示例： - "admin_panel"：访问管理面板 - "execute_multi_account"：多账号管理 - "use_multi_account_button"：多账号控制台按钮 - "export_data"：导出数据 - "view_logs"：查看日志 - "manage_users"：用户管理  错误情况： - session_id缺失或无效：返回{"success": false, "has_permission": false} - 用户未认证：返回{"success": false, "has_permission": false} - permission参数缺失：检查空字符串权限（通常返回false）  安全考虑： - 不泄露权限系统的内部结构 - 失败时统一返回has_permission=false - 不区分"用户不存在"和"无权限"  注意： - 此接口只做检查，不执行实际操作 - 前端应根据返回值动态调整UI - 后端API仍需在执行前再次验证权限 - 游客用户通常无任何特殊权限  使用场景： - 页面加载时检查功能可用性 - 点击操作前的预先验证 - 动态菜单生成
        session_id = request.headers.get("X-Session-ID", "")
        data = request.get_json() or {}
    ...
    def auth_switch_session(): #会话切换API - 在多标签页间切换时更新认证token和cookie。  功能说明： - 支持同一用户在多个浏览器标签页间无缝切换 - 为目标会话生成新的token - 更新浏览器cookie以匹配目标会话 - 预加载目标会话状态（如果存在）  应用场景： - 用户在标签页A登录后，打开标签页B - 用户从会话列表中选择要切换到的会话 - 多窗口协作（如同时查看不同任务）  请求格式： - 方法：POST - 路径：/auth/switch_session - 请求头： * X-Session-ID: 当前活动窗口的会话ID（必需） * Cookie: auth_token=...（用于验证身份） - 请求体： { "target_session_id": "目标会话的UUID" }  响应格式： { "success": true/false, "message": "成功/错误信息", "need_login": true（仅当token失效时） }  响应头： - Set-Cookie: auth_token=新token; Max-Age=3600; HttpOnly; SameSite=Lax  处理流程（5步）： 1. 验证当前用户身份： - 从current_session_id获取用户名 - 检查是否为游客（游客不支持切换） - 验证is_authenticated状态  2. 验证当前token有效性： - 从cookie读取auth_token - 调用token_manager.verify_token()验证 - 如果失效，返回401并要求重新登录  3. 为目标会话生成新token： - 调用token_manager.create_token() - 新token关联到target_session_id - 记录切换事件到日志  4. 更新cookie： - 设置新的auth_token cookie - 1小时过期时间 - HttpOnly防止XSS攻击 - SameSite=Lax防止CSRF  5. 预加载目标会话状态（可选但推荐）： - 从文件加载目标会话的历史状态 - 如果成功，创建Api实例并恢复 - 如果失败，前端访问时会创建新会话  安全特性： - **双重验证**：既验证session_id又验证token - **游客限制**：游客用户不允许切换会话 - **Token刷新**：即使目标已有token，也生成新的 - **自动清理**：无效token会自动清除cookie - **线程安全**：web_sessions_lock保护并发访问  错误情况： - 缺少参数：返回400错误 - 未登录或游客：返回401错误 - Token失效：返回401并清除cookie，设置need_login=true - 目标会话不存在：仍然成功（前端访问时创建）  设计考虑： - **为什么生成新token**： * 增强安全性，每次切换都是新的认证 * 避免token复用导致的安全问题 * 便于追踪用户的会话切换行为  - **为什么预加载状态**： * 提升用户体验，减少加载时间 * 但即使失败也不影响功能 * 前端路由会处理会话创建  注意： - 切换后前端应导航到 /uuid=<target_session_id> - 旧标签页的会话不会被销毁（用户可能返回） - 每个会话都有独立的状态和数据 - Token有效期1小时，过期需重新登录  与单会话强制的关系： - 此功能允许同一用户多会话并存 - 单会话强制针对的是"并发登录设备数" - 会话切换是在已允许的设备内切换  使用流程示例： 1. 用户在标签A登录（会话ID: session-A） 2. 用户打开标签B（自动分配会话ID: session-B） 3. 用户在标签B点击"切换到会话A" 4. 调用此API，target_session_id=session-A 5. 服务器生成新token for session-A 6. 标签B的cookie更新为新token 7. 标签B导航到/uuid=session-A，成功接管会话A的状态
    ...
    def auth_admin_list_users(): #管理员API - 列出所有用户信息。  功能说明： - 获取系统中所有注册用户的列表 - 包含用户名、用户组、创建时间等信息 - 用于管理员查看和管理用户  权限要求： - 必须登录 - 必须具有'manage_users'权限  请求头： - X-Session-ID: 会话ID（必需）  响应格式： 成功：{"success": true, "users": [用户列表]} 失败：{"success": false, "message": "错误信息"}  用户对象结构： { "username": "用户名", "group": "用户组", "created_at": "创建时间", "last_login": "最后登录时间" }  使用场景： - 管理员查看所有用户 - 用户管理界面的数据源 - 审计和统计分析
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_admin_update_user_group(): #管理员API - 修改用户所属的权限组。  功能说明： - 将指定用户分配到新的权限组 - 影响用户的权限范围 - 立即生效，无需重新登录  权限要求： - 必须登录 - 必须具有'manage_users'权限  请求头： - X-Session-ID: 会话ID（必需）  请求体（JSON）： { "target_username": "目标用户名", "new_group": "新权限组名" }  响应格式： 成功：{"success": true, "message": "用户组已更新"} 失败：{"success": false, "message": "错误信息"}  注意事项： - 目标用户组必须存在 - 不能修改自己的用户组（防止锁定） - 更改会立即影响用户权限  使用场景： - 提升/降低用户权限 - 调整用户角色 - 权限管理界面
        session_id = request.headers.get("X-Session-ID", "")
        data = request.get_json() or {}
    ...
    def auth_admin_list_groups(): #管理员API - 列出所有权限组及其权限配置。  功能说明： - 获取系统中定义的所有权限组 - 包含每个组的权限列表 - 用于权限管理和配置  权限要求： - 必须登录 - 必须具有'manage_permissions'权限  请求头： - X-Session-ID: 会话ID（必需）  响应格式： 成功：{"success": true, "groups": {权限组配置}} 失败：{"success": false, "message": "错误信息"}
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_admin_create_group(): #问题7修复：超级管理员：创建权限组（验证必填字段）
        session_id = request.headers.get("X-Session-ID", "")
        data = request.get_json() or {}
    ...
    def auth_admin_update_group(): #问题6修复：超级管理员：更新权限组（支持group_key和group_name参数）
        session_id = request.headers.get("X-Session-ID", "")
        data = request.get_json() or {}
    ...
    def auth_admin_delete_group(): #超级管理员：删除权限组
        session_id = request.headers.get("X-Session-ID", "")
        data = request.get_json() or {}
    ...
    def auth_admin_get_user_permissions(): #管理员：获取用户的完整权限列表
        session_id = request.headers.get("X-Session-ID", "")
        data = request.get_json() or {}
    ...
    def auth_admin_set_user_permission(): #管理员：为用户设置自定义权限（差分化存储）  支持两种模式： 1. 单个权限设置：传递 permission 和 grant 参数 2. 批量权限设置：传递 added_permissions 和 removed_permissions 对象
        session_id = request.headers.get("X-Session-ID", "")
        data = request.get_json() or {}
        target_username = data.get("username", "")
    ...
    def auth_get_config(): #获取认证配置（用于前端显示） 使用 _get_config_value 辅助函数安全地读取配置，避免 ValueError 崩溃
    ...
    def auth_check_uuid_type(): #检查UUID类型：游客UUID、系统账号UUID或未知UUID 用于实现访问控制和会话验证
        data = request.json
    ...
    def auth_2fa_verify(): #测试验证2FA代码（用于用户测试2FA是否工作正常）
        data = request.get_json() or {}
        verification_code = data.get("code", "").strip()
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_2fa_verify_login(): #验证2FA代码并完成登录（用于登录流程中的2FA验证）
        data = request.get_json() or {}
        auth_username = data.get("auth_username", "").strip()
        verification_code = data.get("code", "").strip()
    ...
    def auth_admin_create_user(): #管理员：创建新用户
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_ban_user(): #管理员：封禁用户
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_unban_user(): #管理员：解封用户
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_delete_user(): #管理员：删除用户
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_force_reset_password(): #管理员API：强制重置用户密码。
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def api_admin_update_user_basic_info(username): #问题8修复：管理员或用户本人：更新用户基本信息  功能说明： 允许管理员或用户本人更新基本信息（如昵称）  URL参数: username: 要更新的用户名  请求体参数（JSON）: nickname: 新昵称（可选）  权限要求: - 管理员可以更新任何用户的信息 - 普通用户只能更新自己的信息  返回: 操作结果
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    @login_required
    def auth_admin_update_user_nickname(): #新功能：管理员强制修改用户昵称  权限要求: - 管理员权限 ('manage_users')  请求体参数（JSON）: username: 要更新的用户名 nickname: 新昵称  返回: 操作结果
        try:
    ...
    def auth_admin_update_user_phone(): #问题4修复：管理员更新用户手机号 任务 3/3：落实一个手机号只能绑定一个账号  功能说明： 允许管理员更新用户的手机号（验证码非必需）  请求体参数（JSON）: username: 要更新的用户名 new_phone: 新手机号 sms_code: 验证码（可选，管理员操作不强制）  权限要求: - 管理员权限  返回: 操作结果
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_login_logs(): #获取登录日志（管理员）
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_get_user_school_accounts(): #获取指定认证用户的所有 school_account（管理员或有 auto_fill_password 权限）
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_get_all_users_school_accounts(): #获取所有认证用户的 school_account 列表（管理员或有 auto_fill_password 权限）
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def api_admin_school_account_save(): #保存或添加 School Account（PC端管理面板CRUD支持）。  功能说明： - 允许管理员或有权限的用户保存/更新学校账户信息 - 支持添加新账户和更新现有账户 - 数据包括：用户名、密码、UA（User-Agent）  请求参数（JSON）： - auth_username: 认证用户名（所属用户） - school_username: 学校账户用户名 - password: 学校账户密码 - ua: User-Agent字符串（可选）  权限要求： - 管理员可以保存任何用户的账户 - 普通用户只能保存自己的账户  返回值： - success: 是否成功 - message: 提示信息
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def api_admin_school_account_delete(): #删除 School Account（PC端管理面板CRUD支持）。  功能说明： - 允许管理员或有权限的用户删除学校账户 - 从指定认证用户的账户列表中移除指定的学校账户  请求参数（JSON）： - auth_username: 认证用户名（所属用户） - school_username: 要删除的学校账户用户名  权限要求： - 管理员可以删除任何用户的账户 - 普通用户只能删除自己的账户  返回值： - success: 是否成功 - message: 提示信息
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def api_admin_school_account_update(): #更新 School Account（PC端管理面板CRUD支持）。  功能说明： - 允许管理员或有权限的用户更新学校账户的密码和UA - 支持修改指定认证用户的学校账户信息  请求参数（JSON）： - auth_username: 认证用户名（所属用户） - school_username: 学校账户用户名 - password: 新密码 - ua: 新User-Agent（可选）  权限要求： - 管理员可以更新任何用户的账户 - 普通用户只能更新自己的账户  返回值： - success: 是否成功 - message: 提示信息
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_admin_reset_password(): #重置用户密码（管理员）或修改自己的密码
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_user_update_avatar(): #更新用户头像（URL方式）
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_user_upload_avatar(): #上传用户头像文件（multipart/form-data）
    ...
    def serve_default_avatar(): #为 default_avatar.png 提供根路径和 /static 路径的访问。  问题： - 注册页面 (auth-reg-avatar-preview) hardcodes /static/default_avatar.png (404) - 个人资料 (loadPersonalInfo) 在 avatar_url 为 'default_avatar.png' 时， 会直接请求 /default_avatar.png (404)  解决： 添加这两个路由，统一指向 ./default_avatar.png 文件。
        try:
            import os
    ...
    def serve_avatar(filename): #提供头像图片服务（需要会话认证，管理员可访问）
    ...
    def auth_admin_clear_user_avatar(): #管理员：强制清除用户头像
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_user_update_theme(): #更新用户主题
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_user_details(): #获取用户详细信息
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_get_user_avatar(): #根据用户名获取头像URL（管理员可查询所有用户）  查询参数: - username: 用户名（可选，不提供则返回当前用户）
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_admin_update_max_sessions(): #更新用户最大会话数量（管理员）  请求参数: - username: 目标用户名 - max_sessions: 最大会话数 (1=单会话, >1=指定数量, -1=无限制)
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_user_sessions(): #获取用户的所有会话
        session_id = request.headers.get("X-Session-ID", "")
        api_instance = None
        is_guest = False
        auth_username = ""
    ...
    def auth_user_delete_session(): #删除用户的一个会话
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_user_create_session_persistence(): #创建会话持久化文件（登录状态下）
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def auth_admin_all_sessions(): #管理员：获取所有活跃会话（上帝模式）
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_destroy_session(): #管理员：强制销毁任意会话（上帝模式）
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def auth_admin_audit_logs(): #获取审计日志（管理员）
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未登录"}), 401
    ...
    def sms_send_code(): #发送短信验证码API 功能：向指定手机号发送验证码，用于注册、登录或修改手机号等场景  请求参数： - phone: 手机号（必填） - scene: 使用场景（register/login/modify，默认register）  返回： - success: 是否成功 - message: 提示信息 - code_id: 验证码ID（用于后续验证）
        try:
            config = configparser.ConfigParser()
            config.read("config.ini", encoding="utf-8")
            if (
    ...
    def sms_reply_webhook(): #接收短信宝的用户回复推送（Webhook）  短信宝会通过GET请求推送用户回复内容： - m: 手机号 - c: 回复内容（UTF-8编码）  功能：记录所有用户回复到日志文件，便于后续分析
        try:
    ...
    @login_required
    def sms_test_send(): #短信测试发送API（管理员专用） 功能：向指定手机号发送验证码（可指定或随机），用于测试短信配置是否正确  权限要求：仅管理员可调用  请求参数： - phone: 手机号（必填） - code: 验证码（可选，不填则自动生成随机验证码）  返回： - success: 是否成功 - message: 提示信息 - code: 发送的验证码（仅测试模式下返回，便于管理员验证）
        try:
            current_user = g.user
    ...
    @login_required
    def admin_logs_login_history(): #查看用户登录历史记录  权限要求：管理员或用户本人  参数： - username: 用户名（可选，管理员可查看任意用户，普通用户只能查看自己） - limit: 返回条数（默认100）  返回：登录记录列表
        try:
            current_user = g.user
    ...
    @login_required
    def admin_logs_audit(): #查看用户审计日志（操作记录）  权限要求：管理员  参数： - username: 用户名 - action: 操作类型 - limit: 返回条数（默认100）  返回：审计日志列表
    ...
    def _get_config_value(config, section, key, type_func=str, fallback=None): #辅助函数：从 configparser 安全地获取值，并应用类型转换。 如果 key 不存在，返回 fallback。
    ...
    @login_required
    def admin_config_load(): #加载 config.ini 中的可配置项 权限要求: manage_system
        try:
    ...
    @login_required
    def admin_config_save(): #保存 config.ini 中的可配置项 权限要求: manage_system
        try:
            if not auth_system.check_permission(g.user, "manage_system"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    def log_frontend(): #接收前端日志并保存到后端日志文件
        try:
            data = request.get_json() or {}
    ...
    def auth_check_phone(): #检查手机号是否已被绑定。  请求体: { "phone": "13800138000" }  返回: { "success": true, "is_bound": true/false, "bound_to_user": "username" (如果已绑定) }
        data = request.get_json() or {}
        phone = data.get("phone", "").strip()
    ...
    def user_update_phone(): #用户修改自己的手机号。 - 检查 enable_phone_modification 开关 - 强制验证短信 - 强制解绑其他账号
        try:
            config = configparser.ConfigParser()
            config.read(CONFIG_FILE, encoding="utf-8")
            if (
    ...
    @login_required
    def get_ip_bans(): #获取IP封禁列表  权限要求：管理员 返回：所有IP封禁规则列表
        try:
    ...
    @login_required
    def add_ip_ban(): #添加IP封禁规则  权限要求：管理员 参数： - target: 封禁目标（IP/IP段/城市） - type: 类型（ip/cidr/city） - scope: 范围（all/messages_only）
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def delete_ip_ban(ban_id): #删除IP封禁规则  权限要求：管理员 参数： - ban_id: 封禁规则ID
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    def check_ip_ban_api(): #检查IP封禁状态API  功能说明： 供前端调用，检查当前客户端IP是否被封禁。 前端在切换到留言板标签或发表留言前调用此接口。  请求参数： - scope: 检查范围（'all'或'messages_only'），可选，默认为'all'  返回： { "success": True, "is_banned": True/False  # true表示IP被封禁 }
        try:
            client_ip = request.remote_addr
    ...
    def check_ip_ban(ip_address, scope="all"): #检查IP是否被封禁  参数： - ip_address: 要检查的IP地址 - scope: 检查范围（all或具体功能如messages_only）  返回：True表示被封禁，False表示未被封禁
        if not os.path.exists(IP_BANS_FILE):
            return False
    ...
    @login_required
    def get_sms_config(): #获取短信服务配置  权限要求：管理员 返回：短信服务配置信息（包含三个新增的功能开关）
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def save_sms_config(): #保存短信服务配置  权限要求：管理员 参数：配置对象（enable_sms_service, enable_phone_modification等） 使用 _write_config_with_comments 函数保存配置，确保注释不会丢失
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def check_sms_balance(): #查询短信宝余额  权限要求：管理员 返回：短信余额  修正： 1. 增加了120秒的速率限制，限制期间返回缓存数据。 2. 修正了解析逻辑，以正确处理 "0\n今日条数,剩余条数" 格式的响应。
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def get_sms_history(): #获取短信发送历史记录  权限要求：管理员 查询参数： - date: 按日期过滤（格式：YYYY-MM-DD） - phone: 按手机号过滤 返回：短信发送历史记录列表
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def get_verification_codes(): #获取当前有效的验证码列表  权限要求：管理员 返回：当前有效的验证码列表
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def invalidate_verification_code(): #使指定手机号的验证码失效  权限要求：管理员 参数： - phone: 手机号 返回：操作结果
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def add_manual_verification_code(): #手动添加验证码（模拟发送，用于测试或紧急情况）  权限要求：管理员 参数： - phone: 手机号 - code: 验证码（6位数字） 返回：操作结果
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def get_ssl_info(): #获取当前SSL配置和证书信息。  功能说明： 1. 返回当前SSL配置（是否启用、仅HTTPS模式等） 2. 如果启用了SSL，返回证书详细信息（有效期、颁发者等）  权限要求：管理员  返回示例： { "success": true, "config": { "ssl_enabled": true, "https_only": false, "cert_path": "ssl/fullchain.pem", "key_path": "ssl/privkey.key" }, "cert_info": { "subject": "CN=example.com", "issuer": "CN=Let's Encrypt Authority", "not_before": "2024-01-01T00:00:00", "not_after": "2024-04-01T00:00:00", "is_expired": false } }
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def upload_ssl_certificate(): #上传SSL证书文件和密钥文件。  功能说明： 1. 接收证书文件（PEM格式）和密钥文件 2. 验证文件格式和有效性 3. 保存到ssl/目录 4. 返回上传结果  权限要求：管理员  请求参数（multipart/form-data）： - cert_file: 证书文件（.pem或.crt） - key_file: 密钥文件（.key或.pem）  返回示例： { "success": true, "message": "证书上传成功", "cert_path": "ssl/fullchain.pem", "key_path": "ssl/privkey.key" }
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def update_ssl_config(): #更新SSL配置（不包括证书文件，仅更新配置选项）。  功能说明： 更新config.ini中的SSL配置项，如启用状态、仅HTTPS模式等 注意：更新配置后需要重启服务器才能生效  权限要求：管理员  请求参数（JSON）： { "ssl_enabled": true/false, "https_only": true/false, "cert_path": "ssl/fullchain.pem",  // 可选 "key_path": "ssl/privkey.key"      // 可选 }  返回示例： { "success": true, "message": "配置已更新，重启服务器后生效" }
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    @login_required
    def toggle_ssl(): #快速启用/禁用SSL（开关功能）。  功能说明： 快速切换SSL启用状态，无需修改其他配置 注意：启用时会验证证书是否存在，禁用时不验证  权限要求：管理员  请求参数（JSON）： { "enabled": true/false }  返回示例： { "success": true, "message": "SSL已启用，需要重启服务器", "ssl_enabled": true }
        try:
            if not auth_system.check_permission(g.user, "manage_users"):
                return jsonify({"success": False, "message": "权限不足"}), 403
    ...
    def get_frontend_config(): #辅助函数：读取前端需要的功能开关配置
        config = configparser.ConfigParser()
        config.read(CONFIG_FILE, encoding="utf-8")
    ...
    def favicon(): #返回网站图标（Favicon）文件。  功能说明： - 当浏览器请求网站图标时，返回位于项目根目录的 favicon.ico 文件 - 使用 send_file 函数安全地发送文件到客户端 - 设置正确的 MIME 类型（image/vnd.microsoft.icon）以确保浏览器正确识别  返回值： - 成功：返回 favicon.ico 文件 - 失败：返回 404 错误（如果文件不存在）
        try:
    ...
    def api_shutdown(): #应用退出API端点。  功能说明： - 响应PC端登录页的退出按钮点击事件 - 安全地关闭 Flask + SocketIO 服务器 - 执行必要的清理工作（如关闭数据库连接、保存状态等）  安全考虑： - 此API应该受到权限保护，避免被恶意调用 - 在生产环境中，建议添加身份验证或IP白名单  返回值： - JSON 响应，表示关闭操作已启动
        try:
            ...
            def delayed_shutdown(): #延迟关闭函数：在独立线程中执行关闭操作。  为什么要延迟？ - 让 Flask 有时间完成当前请求的响应发送 - 避免客户端收到连接中断错误
                time.sleep(1)
    ...
    @app.route("/")
    def index(): #首页：显示登录页面，等待用户认证后分配UUID
    ...
    @app.route("/uuid=<uuid>")
    def session_view(uuid): #会话页面：显示应用界面
        uuid_pattern = re.compile(
            r"^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$",
            re.IGNORECASE,
        )
    ...
    def serve_javascript(function_path): #JavaScript 函数动态加载 API 端点  功能说明： 根据请求的函数名，从 JavaScript.js 文件中查找并返回对应的函数代码。 支持浏览器缓存，提升加载速度和性能。  参数： function_path (str): 请求的函数路径，例如 'onlogin' 或 'utils/helper'  返回： - 200: 成功找到函数，返回 JavaScript 代码 - 404: 未找到指定的函数 - 500: 服务器内部错误  缓存策略： - 设置 Cache-Control 头，允许浏览器缓存 1 小时 - 使用 Last-Modified 和 ETag 支持条件请求  示例： GET /JavaScript/onlogin.js --> 返回 onlogin 相关的 JavaScript 函数代码
...
"""
    ...
    def serve_javascript_globals(): #返回 JavaScript 全局变量文件（优先加载）  功能说明： 返回 JavaScript_globals.js 文件，包含所有全局变量声明 此文件必须在主 JavaScript.js 之前加载，避免变量未定义错误  查询参数： - minify: 是否压缩代码（默认 true）  返回： - 200: 成功返回全局变量代码 - 304: 使用缓存版本 - 404: 文件未找到 - 500: 服务器内部错误  使用示例： <script src="/JavaScript_globals.js"></script> <script src="/JavaScript.js"></script>
        try:
    ...
    def serve_css_animation(): #返回 CSS 动画文件（优先加载）  功能说明： 返回 Cascading_Style_Sheets_Web_animation.css 文件，包含所有 CSS 动画样式 此文件必须在主 CSS 文件之前加载，避免样式未定义错误  查询参数： - minify: 是否压缩代码（默认 true）  返回： - 200: 成功返回全局变量代码 - 304: 使用缓存版本 - 404: 文件未找到 - 500: 服务器内部错误
        try:
    ...
    def get_parsed_stylesheet(css_file_path): #获取解析后的 CSS 样式表（实时读取，不使用内存缓存）。  返回 (stylesheet, selectors_list)
    ...
    def get_available_css_selectors(): #返回 Cascading_Style_Sheets.css 文件中所有可用的 CSS 选择器列表。
    ...
    def serve_css(css_path): #CSS 样式分段动态加载 API 端点
    ...
    def serve_html_fragment(fragment_name): #HTML 片段动态加载 API 端点（支持自动压缩，从统一文件读取）
        try:
    ...
    def api_call(method): #API调用端点：将前端调用转发到Python后端
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def execute_js(): #在服务器端Chrome中执行JavaScript代码
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def start_background_task(): #启动后台任务执行
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "会话无效或未登录"}), 401
    ...
    def get_background_task_status(): #获取后台任务状态
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id:
            return jsonify({"success": False, "message": "缺少会话ID"}), 401
    ...
    def stop_background_task(): #停止后台任务
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "会话无效或未登录"}), 401
    ...
    def get_ip_location(ip_address): #获取IP地址的地理位置信息 (带1天缓存)  参数: ip_address (str): IP地址字符串（支持IPv4和IPv6）  返回: str: 地理位置信息字符串，格式为去重后的"info1 info2 info3 isp" 获取失败返回"未知"  实现说明: - 优先从内存和文件缓存 (logs/ip_location_cache.json) 中读取 - 缓存有效期为1天 (CACHE_DURATION_SECONDS) - 缓存未命中或过期时，调用第三方API（https://api.vore.top/api/IPdata）查询 - 查询成功后更新内存缓存和文件缓存 - 异常情况返回'未知'，确保不影响主业务流程
        if not ip_address:
            return "未知"
    ...
    def validate_amap_key(): #验证高德地图API Key的有效性  请求体: { "key": "高德地图API Key字符串" }  返回: { "success": True/False, "message": "验证结果消息" }  实现说明: - 调用高德地图的地理编码API进行验证 - 使用固定坐标（天安门）作为测试参数 - 根据返回的status字段判断Key是否有效
        try:
            data = request.get_json() or {}
    ...
    def get_reminders_list(): #获取所有提醒列表  功能说明: - 读取reminders.json文件中的所有提醒数据 - 返回提醒列表供前端显示 - 仅管理员有权查看和管理提醒  返回格式: { "success": True/False, "reminders": [提醒对象列表], "message": "错误消息（仅失败时）" }  提醒对象格式: { "id": "唯一标识符", "title": "提醒标题", "message": "提醒内容", "start_time": "开始时间 HH:MM", "end_time": "结束时间 HH:MM", "enabled": true/false, "created_at": 创建时间戳, "updated_at": 更新时间戳 }
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def add_reminder(): #添加新提醒  请求体: { "title": "提醒标题", "message": "提醒内容", "start_time": "19:00", "end_time": "20:00", "enabled": true }  返回: { "success": True/False, "message": "操作结果消息", "reminder_id": "新创建的提醒ID（成功时）" }  验证规则: - 标题和内容不能为空 - 时间格式必须为 HH:MM（24小时制） - 标题长度不超过50字符 - 内容长度不超过500字符
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未授权访问"}), 401
    ...
    def update_reminder(): #更新现有提醒  请求体: { "id": "提醒ID", "title": "新标题", "message": "新内容", "start_time": "19:00", "end_time": "20:00", "enabled": true }  返回: { "success": True/False, "message": "操作结果消息" }  说明: - 必须提供提醒ID - 其他字段可选，未提供的字段保持原值 - 更新updated_at时间戳
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未授权访问"}), 401
    ...
    def delete_reminder(): #删除提醒  请求体: { "id": "提醒ID" }  返回: { "success": True/False, "message": "操作结果消息" }
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未授权访问"}), 401
    ...
    def check_reminders(): #检查当前时间是否有需要显示的提醒  功能说明: - 获取当前时间（HH:MM格式） - 遍历所有启用的提醒 - 判断当前时间是否在提醒的时间范围内 - 支持跨天时间段判断（如 20:00 到次日 08:00）  返回: { "success": True, "reminders": [当前需要显示的提醒列表] }  时间判断逻辑: - 正常时间段（start_time < end_time）： 当前时间 >= start_time 且 当前时间 < end_time - 跨天时间段（start_time > end_time）： 当前时间 >= start_time 或 当前时间 < end_time
        try:
            from datetime import datetime
    ...
    def _is_time_in_range(current_time, start_time, end_time): #辅助函数：判断当前时间是否在指定的时间范围内  参数: current_time (str): 当前时间，格式为 HH:MM start_time (str): 开始时间，格式为 HH:MM end_time (str): 结束时间，格式为 HH:MM  返回: bool: True表示在范围内，False表示不在范围内  逻辑说明: - 情况1：正常时间段（start_time < end_time） 例如：09:00 到 17:00 判断：current_time >= start_time 且 current_time < end_time  - 情况2：跨天时间段（start_time > end_time） 例如：20:00 到次日 08:00 判断：current_time >= start_time 或 current_time < end_time  示例: - _is_time_in_range("19:30", "19:00", "20:00") -> True - _is_time_in_range("20:30", "20:00", "08:00") -> True （跨天） - _is_time_in_range("07:30", "20:00", "08:00") -> True （跨天） - _is_time_in_range("09:00", "20:00", "08:00") -> False
        try:
            def time_to_minutes(time_str): #将HH:MM格式的时间转换为从00:00开始的分钟数
    ...
    def get_captcha(): #获取验证码接口 【本地验证码生成器】  功能说明: - 使用本地MicroPixelCaptcha生成器生成验证码（替代不稳定的第三方API） - 生成唯一的验证码ID，将ID和答案存储在JSON文件中 - 返回验证码的HTML内容、验证码ID和尺寸给前端显示  返回格式: { "success": True/False, "html": "验证码HTML内容（用于在前端显示图片）", "captcha_id": "唯一的验证码ID", "width": 验证码宽度（像素）， "height": 验证码高度（像素）， "message": "错误消息（仅失败时返回）" }  安全说明: - 验证码答案存储在服务器端JSON文件中，不会返回给前端 - 使用UUID生成唯一的验证码ID，防止冲突 - 验证码文件存储在logs/captchas目录下 - 验证码有效期10分钟，过期自动清理
        session_id = request.headers.get("X-Session-ID", "")
    ...
    def get_captcha_html_page(captcha_id): #获取验证码HTML页面（用于iframe嵌入）
...
<body>
    ...
    def verify_captcha(captcha_id, user_input): #验证验证码辅助函数  参数: captcha_id (str): 验证码ID user_input (str): 用户输入的验证码  返回: tuple: (是否验证成功: bool, 错误消息: str) 成功返回 (True, "") 失败返回 (False, "具体错误原因")  验证逻辑: 1. 根据验证码ID从JSON文件中读取验证码数据 2. 检查验证码是否过期 3. 不区分大小写比对用户输入与存储的验证码答案 4. 验证后立即删除验证码文件（一次性使用）  安全特性: - 验证码使用后立即失效（防止重放攻击） - 不区分大小写，提升用户体验 - 验证失败也会删除验证码，需要重新获取 - 支持验证码过期机制
    ...
    def get_captcha_history(): #获取验证码请求历史记录（管理员功能）  权限要求：管理员 查询参数： - date: 日期（格式：YYYYMMDD），默认为今天 - limit: 返回记录数量限制，默认100 - status: 状态过滤（created, verified_success, verified_failed, expired）  返回格式： { "success": True/False, "data": [验证码历史记录列表], "total": 总记录数, "message": "错误消息" }
        session_id = request.headers.get("X-Session-ID", "")
        if not session_id or session_id not in web_sessions:
            return jsonify({"success": False, "message": "未授权访问"}), 401
    ...
    @login_required
    def get_captcha_detail(captcha_id): #获取单个验证码的详细信息  权限要求：管理员 (view_captcha_history)
        try:
    ...
    @login_required
    def save_captcha_settings(): #【本地验证码】保存验证码生成器设置到config.ini  权限要求：管理员 (modify_config)  功能说明: - 将验证码生成参数（length, scale_factor, noise_level）保存到config.ini的[Captcha]节 - 参数会经过严格的范围验证，防止无效或危险的配置 - 保存后立即生效，下次生成验证码时使用新参数  请求格式 (JSON): { "length": 4,           # 验证码字符数（3-6） "scale_factor": 2,     # 像素细分倍数（2-4） "noise_level": 0.08    # 噪点比例（0.0-0.3） }  返回格式: { "success": True/False, "message": "设置保存成功" / "错误信息" }  安全说明: - 只有管理员才能修改验证码设置 - 所有参数都会进行范围检查，防止异常值 - 使用_write_config_with_comments保存，保留注释
        try:
    ...
    @login_required
    def test_generate_captcha(): #【本地验证码】测试生成验证码（用于预览效果）  权限要求：管理员 (modify_config)  功能说明: - 使用指定的参数临时生成一个验证码，用于预览效果 - 不会将验证码保存到数据库或记录历史 - 返回验证码的HTML和答案（仅用于测试预览） - 管理员可以在"验证码设置"面板中调整参数并实时预览  请求格式 (JSON): { "length": 4,           # 验证码字符数（3-6） "scale_factor": 2,     # 像素细分倍数（2-4） "noise_level": 0.08    # 噪点比例（0.0-0.3） }  返回格式: { "success": True/False, "code": "ABC1",           # 验证码答案（仅用于测试预览） "html": "<div>...</div>", # 验证码HTML（用于前端显示） "message": "生成成功" / "错误信息" }  安全说明: - 只有管理员才能测试生成验证码（需要modify_config权限） - 测试生成的验证码不会被记录或用于实际登录 - 返回的code仅用于预览效果，实际使用时不返回
        try:
            if not auth_system.check_permission(g.user, "modify_config"):
                logging.warning(
    ...
    @app.route("/health")
    def health(): #健康检查端点（已增强）  功能说明： - 提供服务器的健康状态和运行统计信息 - 用于监控系统、负载均衡器或运维人员检查服务是否正常  返回的信息包括： 1. status: 服务器状态（ok表示正常） 2. uptime_seconds: 服务器运行秒数（原始数值） 3. uptime_formatted: 服务器运行时间（人类可读格式，如"2天3小时15分钟"） 4. active_memory_sessions: 活跃的内存会话数 5. active_background_tasks: 活跃的后台任务数 6. current_thread_chrome_contexts: 当前线程的Chrome上下文数 7. response_time_ms: 请求响应延迟（毫秒）
        ...
        def format_uptime(seconds): #将秒数转换为人类可读的时间格式。  参数: seconds: 总秒数  返回: 格式化的字符串，如 "2天3小时15分钟30秒"
    ...
    @socketio.on("connect")
    def handle_connect(): #前端 JS 连接时会触发
    ...
    @socketio.on("join")
    def handle_join(data): #【修复】增强健壮性：处理 data 可能为字符串的情况
    ...
    def _emit_verification_codes_update(session_id): #辅助函数：向指定会话推送验证码列表更新事件
        if not session_id or not socketio:
            logging.debug(
    ...
    def cleanup_sessions(): #定期清理超过24小时无活动的会话
        while True:
    ...
    @app.before_request
    def handle_forwarded_proto(): #处理反向代理（如Nginx）转发的请求头。  功能说明： 1. 检查X-Forwarded-Proto头，识别原始请求是HTTP还是HTTPS 2. 检查X-Forwarded-For头，获取真实客户端IP 3. 如果启用了https_only模式且检测到HTTP请求，重定向到HTTPS  这个函数在每个请求处理之前自动执行。
    ...
    @app.after_request
    def add_security_headers(response): #为所有响应添加安全相关的HTTP头。  功能说明： 添加多个安全响应头，提升应用的安全性： 1. HSTS (Strict-Transport-Security): 强制浏览器使用HTTPS 2. X-Content-Type-Options: 防止MIME类型嗅探攻击 3. X-Frame-Options: 防止点击劫持攻击 4. X-XSS-Protection: 启用浏览器XSS防护 5. Referrer-Policy: 控制Referer头的发送策略  这个函数在每个响应发送之前自动执行。
...
def main(): #主函数，启动Web服务器模式（已弃用桌面模式）
